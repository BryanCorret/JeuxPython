...
class Client():
    ...
    def reception(self):
        ind_0=self.reserve.find(self.fin_de_message) #on vérifie que la réserve ne contient pas déjà le message
        if ind_0!=-1:
            ...
            while not ok: # on lit sur la socket jusqu'à obtenir un message contenant le caractère '\0'
                ...
                if ind_0!=-1: #le message recu contient un caractère '\0'
                    ok=True
                    msg+=msg_comp[:ind_0] # on met tout ce qui est avant le '\0' dans le message
                    self.reserve=msg_comp[ind_0+1:] # et le reste dans la réserve
                else:
                    msg+=msg_comp
        return msg
...
class ClientCyber(Client):
    ...
    def enregistrement(self,nom_client,type_client):
        self.nom_client=nom_client.replace(self.separateur,"_",-1).replace("\n","_")
    ...
    def prochaine_commande(self):
       ...
       # traitement du message
        ...
        le_jeu=None
        try:
            le_jeu=jeu.creer_jeu_from_str(msg[fin_entete+1:])
        except:
    ...
    def envoyer_jeu(self,jeu_str,num_joueur):
